  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-800">
      <main className="max-w-[1400px] mx-auto flex flex-col lg:flex-row min-h-screen">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
            {currentProducts.map((p) => (
              <button
                key={p.id}
                onClick={() => addItem(p)}
                className="group p-6 border border-slate-100 bg-[#FBFDFA] hover:border-[#2D6A4F]/20 transition-all text-left flex flex-col justify-between aspect-video"
              >
                <div>
                  <span className="text-[9px] text-[#2D6A4F]/40 font-mono mb-1 block uppercase">
                    SKU-{p.id.toString().slice(-4)}
                  </span>
                  <h3 className="text-sm font-bold text-slate-900 group-hover:text-[#2D6A4F] transition-colors uppercase tracking-tight">
                    {p.name}
                  </h3>
                </div>
                <p className="text-xs text-slate-400 italic">
                  Base Cost: ₹{p.costPrice}
                </p>
              </button>
            ))}
          </div>
        ...
        <aside className="w-full lg:w-100 bg-[#FBFDFA] p-8 lg:p-12 flex flex-col">
          <div className="mb-10">
